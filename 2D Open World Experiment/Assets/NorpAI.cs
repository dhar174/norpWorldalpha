...
public class NorpAI : MonoBehaviour {
    ...
    public Vector2[] tVectors = new Vector2[10];
    public Vector2[] tDirections = new Vector2[10];

    public int counterSleeping;

    public List<Collider2D> circleListCopy = new List<Collider2D>(10);
    ...
    public Transform[] targetList = new Transform[10];
    private void Awake()
    {
    ...
    public void TargetList(Transform[] targets)
    {
        ...
        //ArrayIterator<Transform> start = targets.Begin();
        //Transform val = start.GetCurrent();
        // ArrayIterator<Transform> second = start.GetNext();
        //  ArrayIterator<Transform> end = targets.End();
        //  second.Reverse(end);

        // start.ForEach(end, AddIt(e=);
        //~{ };
    }
   ...
   // }
    ...
    {
        var i = 0;
        foreach(Transform ti in targetList)
        {
            //i = System.Array.IndexOf(targetList, ti);
            tVectors[i] = ti.position - this.gameObject.transform.position;
    ...
    public Transform[] FindNearestTenTransforms()
    {
        ...
        Transform[] tMin = new Transform[10];
                ...
                if (Vector2.Distance(g.transform.position, transform.position) <= stopDist)
                {
                    if (g.gameObject.GetComponent<NPCAI>().openToPlay)
                    {
                        if (gameObject.GetComponent<NPCAI>().openToPlay)
                        {
                            ...
                            currentTarget = g;
                            g.gameObject.GetComponent<NorpAI>().state = State.PLAYFUL;
                            g.gameObject.GetComponent<NorpAI>().currentNearest = this.gameObject.transform;
                            g.gameObject.GetComponent<NorpAI>().currentTarget = this.gameObject;
                            hasTarget = true;
                            openToPlay = true;
                            state = State.PLAYFUL;
